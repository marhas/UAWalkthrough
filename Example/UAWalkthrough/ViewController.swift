...
class ViewController: UIViewController {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 2) { [unowned self] in
            self.startWalkthrough(withSettings: self.walkthroughSettings)
        }
    }
...
extension ViewController: WalkthroughProvider {
    var walkthroughItems: [WalkthroughItem] {
        return [
            StandaloneItem(text: "This is a demo of the UAWalkthrough pod, which can be used eg to introduce your app to your user."),
